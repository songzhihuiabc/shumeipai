...
export default class NanoDLPService {
  ...
  async getZAxisHt(){
    ...
    var data = new Promise((resolve, reject) => {
      request1({
        uri: `${this.serverURL}/z-axis/info`,
        method: "GET",
        headers:  { Cookie: `${this.auth ?  this.session : ""}`},
        //type: type,
        json: "json",
  ...
  async getSlicer(){
    var slicerdata = await this._request("/slicer");
    return slicerdata;
  }


  async autoShutdown(){
    return await this._request("/printer/auto-shutdown/enable");
  }

  async unautoShutdown(){
    return await this._request("/printer/auto-shutdown/disable");
  }
